...
@export var basic_attack_hit_frame: int = 3
@export var basic_attack_extra_range: float = 18.0
@export var hurt_pose_duration: float = 0.18
@export var mutant_seed_reveal_hold_time: float = 0.55
...
func _ready() -> void:
	...
	# Boss tears down farm structures much faster than normal enemies.
	...
	if body_sprite == null:
		print(
			"[Tutorial Boss Animation] BodySprite is missing from ",
			"TutorialBoss.tscn."
		)
		return
...
func perform_attack_on_target(target: Node2D) -> void:
	if is_death_animation_playing or is_basic_attack_active:
		return
...
func show_hit_feedback() -> void:
	...
	# Keep the six-frame root slam readable if the boss is hit mid-attack.
...
func die() -> void:
	...
	print(
		enemy_name,
		" defeated. Playing Mutant Seed reveal."
	)
	...
	_award_mutant_seed()
	_notify_tutorial_manager()

	queue_free()


func _award_mutant_seed() -> void:
	...
	if (
		...
		print(
			"[Tutorial Boss] Awarded ",
			mutant_seed_reward_amount,
			" Mutant Seed."
		)


func _notify_tutorial_manager() -> void:
...
func _update_walk_animation() -> void:
	...
	# The boss has no gameplay idle animation.
	# Hold the first walk frame while stopped.
...
func _set_facing_from_direction(direction: Vector2) -> void:
	...
	# The supplied boss artwork faces left.
...
func _play_looping_animation(
	...
	if not body_sprite.sprite_frames.has_animation(
		...
		print(
			"[Tutorial Boss Animation] Missing animation: ",
			animation_name
		)
		return false
...
func _start_action_animation(
	...
	print(
		"[Tutorial Boss Animation] Playing: ",
		animation_name
	)
